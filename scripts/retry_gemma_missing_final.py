#!/usr/bin/env python3
"""
Retry missing Gemma questions to complete the evaluation
"""
...
def main():
    ...
    gemma_results_path = repo_root / "results" / "openrouter" / "gemma_text_only_batched_20260311_144604.json"
    print(f"\nLoading previous results from: {gemma_results_path}")
    with open(gemma_results_path) as f:
    ...
    missing_ids = sorted(set(all_questions_dict.keys()) - answered_ids)
    print(f"\nTotal questions: {len(all_questions_dict)}")
    ...
    missing_questions = [all_questions_dict[qid] for qid in missing_ids]
    ...
    print(f"\nRetrying {len(model_questions)} missing questions...")
    ...
    all_answers = {}
    total_usage = {
        "prompt_tokens": previous_results.get('usage', {}).get('prompt_tokens', 0),
        "completion_tokens": previous_results.get('usage', {}).get('completion_tokens', 0),
        "total_tokens": previous_results.get('usage', {}).get('total_tokens', 0)
    }
    ...
    for answer in previous_results['answers']:
        final_answers[answer['id']] = answer['answer']

    for qid, answer in all_answers.items():
        final_answers[qid] = answer

    print(f"\nFinal evaluation complete.")
    print(f"Previously answered: {len(previous_results['answers'])}")
    print(f"Newly answered: {len(all_answers)}")
    print(f"Total unique answers: {len(final_answers)}/{len(all_questions_dict)}")
    ...
    results = {
        ...
        "num_questions": len(all_questions_dict),
        "num_answered": len(final_answers),
        "usage": total_usage,
        "answers": ResponseParser().format_for_evaluation(final_answers)
    }
    ...
    if len(final_answers) == len(all_questions_dict):
        print(f"\n🎉 SUCCESS! All {len(all_questions_dict)} questions answered!")
    else:
        still_missing = len(all_questions_dict) - len(final_answers)
        print(f"\n⚠️  Still missing {still_missing} questions")
